        ...
        <div class="content">
            <ul class="box" id="box1" v-if="(chats.length > 0)">
                <li class="chat" v-for="(chat, index) in chats" :key="index">
                    <router-link :to="'/professor/mensagens/' + (chat.entity.type).toLowerCase() + '/' + chat.entity.email">
...
import { getLastMessages } from '../../services/api/professor';
...
export default {
    ...
    methods: {
        async getChats() {
            ...
            const response = await getLastMessages(this.professor.token);
